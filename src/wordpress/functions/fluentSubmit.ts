...
const getWordPressUploadUrl = () => {
  ...
  const customUploadPath =
    process.env.FLUENT_UPLOAD_PATH || "/wp-json/headless/v1/fluent-upload";
  return `${baseUrl}${
    customUploadPath.startsWith("/") ? customUploadPath : `/${customUploadPath}`
  }`;
};
...
export async function handleFluentSubmitRequest(request: Request) {
  const referer =
